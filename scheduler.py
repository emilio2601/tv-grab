...
def set_up_recording(rec):
    if rec.tunerID == 1: #That's you, Hauppauge 950Q
        if rec.format == "raw":
            ...
            adapter = multiprocessing.Process(target=azap, parameters=(rec.channel))
            adapter.start()  #At this point, we've set up the adapter but haven't started recording

            recorder = multiprocessing.Process(target=record, parameters=("/dev/dvb/adapter0/dvr0", title))
            recorder.start()
